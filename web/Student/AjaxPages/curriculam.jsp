...
<div id="centerwrapper">
    <ul class="column">
    <%while(rs.next()){
        sql="select count(distinct(a.examid)) from marks m,assessment_master a where a.examid=m.examid and a.section='"+session.getAttribute("section")+"' and a.subject_id='"+rs.getString(1)+"'  group by a.section;";
       // out.print(sql);
    ...
    <li>
        <div class="block" >
            <ul>
                ...
                <li>Course <a href="#" onclick="getCourseOutline('<%=rs.getString(1)%>','<%=session.getAttribute("section")%>')">Outline</a>,<a href="#" onclick="getCourseCoverage('<%=rs.getString(1)%>','<%=session.getAttribute("section")%>')">Coverage</a>,<a href="#" onclick="getCourseProgress('<%=rs.getString(1)%>','<%=session.getAttribute("section")%>')">Progress</a></li>
            </ul>
